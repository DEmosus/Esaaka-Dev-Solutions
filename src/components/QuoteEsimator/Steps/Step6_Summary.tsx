...
export default function Step6() {
  ...
  const handleContinueToContact = () => {
    localStorage.setItem(
      "quoteData",
      JSON.stringify({ ...data, totalEstimate })
    );
  ...
  return (
    <section aria-labelledby="summary-heading" className="p-4">
      <h2 id="summary-heading" className="text-xl font-bold mb-4">
        Summary & Estimate
      </h2>

      <div className="space-y-2 text-sm text-gray-300">
        <p>
          <strong>Solution:</strong> {data.solutionType}
        </p>
        <p>
        ...
        {showBreakdown && (
          <div className="mt-3 text-sm text-gray-400 bg-gray-800 p-4 rounded-lg">
            ...
            <p>
              ...
              $
              {(
                data.features.length * (config?.perFeature ?? 500)
              ).toLocaleString()}
            </p>
      ...
      <div className="mt-6">
        ...
        <div className="max-h-[250px] overflow-y-auto bg-gray-800 border border-gray-700 rounded-xl p-4 text-sm text-gray-300 whitespace-pre-wrap">
          {loading ? (
            ...
            suggestions || (
              <span className="text-red-400">No suggestions available.</span>
            )
          )}
        </div>
      </div>
